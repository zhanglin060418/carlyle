<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="失效时间" prop="expiredAt">
        <el-date-picker clearable
          v-model="queryParams.expiredAt"
          type="date"
    ...
    <el-table v-loading="loading" :data="codeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="失效时间" align="center" prop="expiredAt" width="180">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="手机号码" prop="phoneNo">
          <el-input v-model="form.phoneNo" placeholder="请输入手机号码" />
        </el-form-item>
        <el-form-item label="验证码" prop="verifyCode">
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.phoneNo)
    ...
    handleUpdate(row) {
      this.reset();
      const phoneNo = row.phoneNo || this.ids
      getCode(phoneNo).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const phoneNos = row.phoneNo || this.ids;
      this.$modal.confirm('是否确认删除【请填写功能名称】编号为"' + phoneNos + '"的数据项？').then(function() {
        return delCode(phoneNos);
      }).then(() => {
        this.getList();
