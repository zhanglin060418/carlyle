    ...
    <el-table v-loading="loading" :data="historyList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="交易类型" align="center" prop="transactionType" >
        <template slot-scope="scope">
          <span v-if="scope.row.transactionType == 'Child_First_Purchase_Reward'">下级首单奖励</span>
      ...
      <el-table-column label="返佣" align="center" prop="amount">
        <template slot-scope="scope">
          {{ parseFloat(scope.row.amount)/100 || 0}}
        </template>
      </el-table-column>
      <el-table-column label="来源" align="center" prop="orUserName" />
      <el-table-column label="返佣时间" align="center" prop="transactionDate">
        <template slot-scope="scope">
...
<script>
import { listCommission } from "@/api/system/history";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCommission(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
