<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="银行业权" prop="name">
        <el-input
...
<!--      </el-col>-->
    ...
    <el-table v-loading="loading" :data="bankList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="银行ID" align="center" prop="id" />
      <el-table-column label="银行业权" align="center" prop="name" />
      <el-table-column label="银行代码" align="center" prop="code" />
    ...
    <el-dialog :title="title" :close-on-click-modal="false" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="银行业权" prop="name">
          <el-input v-model="form.name" placeholder="请输入银行业权" />
        </el-form-item>
        <el-form-item label="银行代码" prop="code">
          <el-input v-model="form.code" placeholder="请输入银行代码" />
        </el-form-item>
        <el-form-item label="介绍" prop="description">
          <el-input v-model="form.description" placeholder="请输入银行代码" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        name: [
          { required: true, message: "银行业权不能为空", trigger: "blur" }
        ],
        code: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除银行名单编号为"' + ids + '"的数据项？').then(function() {
        return delBank(ids);
      }).then(() => {
        this.getList();
